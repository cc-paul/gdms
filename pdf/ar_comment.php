...
$sql    = "
   SELECT
    ...
    FROM
        omg_comments a
    WHERE
        a.commentMotherID = '".$pref."-generalcomment'
    ORDER BY
        a.dateCreated DESC;
";
$result = mysqli_query($con,$sql);
...
while ($row  = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Row(array(
     ...
     $sql_comment_header    = "
        ...
        AND 
            CONCAT(SUBSTRING_INDEX(a.commentMotherID, '-', 1),'-',SUBSTRING_INDEX(a.commentMotherID, '-', -1))  = CONCAT('".$pref."','-row$row_count2')
        ORDER BY
            a.dateCreated DESC
        LIMIT 1;
    ";
